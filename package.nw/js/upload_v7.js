...
document.addEventListener('DOMContentLoaded', function () {
	...
	window.__UPLOAD_ENGINE__ = 'v7-cli';
...
function runCliUploadFlow(inoPath) {
	...
	try {
		...
		appendUploadState(response, "[V7-CLI] " + Blockly.Msg.BUTTON_UPLOAD_START + "\n");
...
function bindUploadDialogButtonsV7(inoPath) {
	var $widget = $("#dialog_upload").dialog("widget");
	var $buttons = $widget.find(".ui-dialog-buttonpane button");
	if ($buttons.length < 4) {
		return;
	}
	$buttons.eq(0).off('click').on('click', function () { $("#dialog_upload").dialog("close"); });
	$buttons.eq(1).off('click').on('click', function () {
		document.getElementById('upload_code').style.display = "none";
		document.getElementById('upload_state').style.display = "block";
	});
	$buttons.eq(2).off('click').on('click', function () {
		document.getElementById('upload_code').style.display = "block";
		document.getElementById('upload_state').style.display = "none";
	});
	$buttons.eq(3).off('click').on('click', function () {
...
function startUploading(inoPath) {
	...
	var opt = {
		...
		title: Blockly.Msg.BUTTON_UPLOAD_TITLE + " V7-CLI"
	};
