...
document.addEventListener('DOMContentLoaded', function () {
	...
	window.__UPLOAD_ENGINE__ = 'v8-cli';
...
function runCliUploadFlow(inoPath) {
	...
	try {
		...
		appendUploadState(response, "[V8-CLI] " + Blockly.Msg.BUTTON_UPLOAD_START + "\n");
		...
		var sketchDir = tmpInoDir;
		var buildPath = tmpBuildDir;
		appendUploadState(response, "\n[1/4] Checking toolchain...\n");
		logUpload('start preflight');
		ensureArduinoCliAndCore(boardFqbn, response, function (preflightErr) {
			if (preflightErr) {
				logUpload('preflight failed: ' + (preflightErr && preflightErr.message ? preflightErr.message : String(preflightErr)));
				return;
			}
			appendUploadState(response, "\n[2/4] Compiling with arduino-cli...\n");
			logUpload('start compile');

			runArduinoCliWithOutput(
				['compile', '--fqbn', boardFqbn, '--build-path', buildPath, sketchDir],
				response,
				function (compileErr, compileCode) {
					if (compileErr || compileCode !== 0) {
						appendUploadState(response, "\nCompile failed.\n");
						logUpload('compile failed code=' + compileCode + ' err=' + (compileErr && compileErr.message ? compileErr.message : 'none'));
						return;
					}

					appendUploadState(response, "\n[3/4] Uploading with arduino-cli...\n");
					logUpload('start upload');
					runArduinoCliWithOutput(
						['upload', '--fqbn', boardFqbn, '-p', selectedPort, sketchDir],
						response,
						function (uploadErr, uploadCode) {
							if (!uploadErr && uploadCode === 0) {
								appendUploadState(response, "\n[4/4] Finish\n");
								logUpload('upload success');
							} else {
								appendUploadState(response, "\nUpload failed.\n");
								logUpload('upload failed code=' + uploadCode + ' err=' + (uploadErr && uploadErr.message ? uploadErr.message : 'none'));
							}
						}
					);
				}
...
function ensureUploadToolbarV8(inoPath) {
	var dialog = document.getElementById('dialog_upload');
	if (!dialog) return;
	var toolbar = document.getElementById('upload_toolbar_v8');
	if (!toolbar) {
		toolbar = document.createElement('div');
		toolbar.id = 'upload_toolbar_v8';
		toolbar.style.textAlign = 'right';
		toolbar.style.paddingTop = '8px';
		toolbar.innerHTML =
			'<button type="button" id="upload_btn_close_v8"></button> ' +
			'<button type="button" id="upload_btn_state_v8"></button> ' +
			'<button type="button" id="upload_btn_code_v8"></button> ' +
			'<button type="button" id="upload_btn_run_v8"></button>';
		dialog.appendChild(toolbar);
	}

	var btnClose = document.getElementById('upload_btn_close_v8');
	var btnState = document.getElementById('upload_btn_state_v8');
	var btnCode = document.getElementById('upload_btn_code_v8');
	var btnRun = document.getElementById('upload_btn_run_v8');
...
function runArduinoCliWithOutput(args, response, onExit) {
	...
	var p = spawn('arduino-cli', args, { shell: false });
...
function startUploading(inoPath) {
	...
	var opt = {
		...
		height: 500,
		buttons: [],
		title: Blockly.Msg.BUTTON_UPLOAD_TITLE + " V8-CLI"
	};
	...
	document.getElementById('upload_state').value = "[UP-V8 READY]\n按右下角「下載」開始。\n";
}
