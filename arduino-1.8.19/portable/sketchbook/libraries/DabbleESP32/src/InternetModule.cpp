...
float InternetModule::getFieldData(String KEY,uint8_t fieldNumber,long timeout)
{
  ...
  if (feedindex != -1) {
		...
		if (fieldNumber == 1) {
			...
			int fieldindex = mainString.indexOf("field2", feedindex);
		} else if (fieldNumber == 3) {
			int fieldindex = mainString.indexOf("field3", feedindex);
		} else if (fieldNumber == 4) {
			int fieldindex = mainString.indexOf("field4", feedindex);
		} else if (fieldNumber == 5) {
			int fieldindex = mainString.indexOf("field5", feedindex);
		} else if (fieldNumber == 6) {
			int fieldindex = mainString.indexOf("field6", feedindex);
		} else if (fieldNumber == 7) {
			int fieldindex = mainString.indexOf("field7", feedindex);
		} else if (fieldNumber == 8) {
			int fieldindex = mainString.indexOf("field8", feedindex);
		}
    if (fieldindex != -1) {
			float fieldValue; 
      for (int k=9; k<100; k++) {
				String value1string = mainString.substring(fieldindex+9,fieldindex+k+1);
				fieldValue = value1string.toFloat();
				k++;
			}
			return fieldValue;
		}
