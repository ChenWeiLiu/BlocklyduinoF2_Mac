...
uint32_t ESP8266::recv(uint8_t *buffer, uint32_t buffer_size, uint32_t timeout , String type)
{
    return recvPkg(buffer, buffer_size, NULL, timeout, NULL , type);
}


uint32_t ESP8266::recv(uint8_t mux_id, uint8_t *buffer, uint32_t buffer_size, uint32_t timeout, String type)
{
    uint8_t id;
    uint32_t ret;
    ret = recvPkg(buffer, buffer_size, NULL, timeout, &id , type);
    if (ret > 0 && id == mux_id) {
        return ret;
    }
    return 0;
...
uint32_t ESP8266::recv(uint8_t *coming_mux_id, uint8_t *buffer, uint32_t buffer_size, uint32_t timeout,String type)
{
    return recvPkg(buffer, buffer_size, NULL, timeout, coming_mux_id , type);
}
...
uint32_t ESP8266::recvPkg(uint8_t *buffer, uint32_t buffer_size, uint32_t *data_len, uint32_t timeout, uint8_t *coming_mux_id , String type)
{
    String data;
    char a;
...
bool ESP8266::eATCIFSR_IP(String &list)
{
    rx_empty();
    m_puart->println("AT+CIFSR");
    return recvFindAndFilter("OK", "+CIFSR:STAIP,", "+CIFSR:STAMAC", list);
}
