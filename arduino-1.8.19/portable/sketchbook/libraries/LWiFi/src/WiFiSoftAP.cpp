...
void setupWiFiConfig(const char* ssid, const char* passphrase, int channel) {
    ...
    if(NULL == passphrase) {
		...
		// WEP encrypted
		/*uint8_t password_length = strlen(passphrase);
		const char *password = passphrase;
		wifi_wep_key_t wep_key;
		wep_key.wep_tx_key_index = 0;
		strncpy((char*)wep_key.wep_key[0], password, password_length);
		wep_key.wep_key[0][password_length] = '\0';
		wep_key.wep_key_length[0] = password_length;
		wifi_config_set_security_mode(WIFI_PORT_AP, WIFI_AUTH_MODE_OPEN, WIFI_ENCRYPT_TYPE_WEP_ENABLED);
		wifi_config_set_wep_key(WIFI_PORT_AP, &wep_key);*/
    }
}
...
bool WiFiClass::softAP(const char* ssid, const char* passphrase, int channel) {
	...
	wifi_config_set_wireless_mode(WIFI_PORT_AP,WIFI_PHY_11BGN_MIXED);
	wifi_config_set_country_region(WIFI_BAND_2_4_G,1);
	wifi_config_set_channel(WIFI_PORT_AP,channel);
	wifi_config_set_bandwidth(WIFI_PORT_AP,WIFI_IOT_COMMAND_CONFIG_BANDWIDTH_40MHZ);
...
bool WiFiClass::softAP_5G(const char* ssid, const char* passphrase, int channel) {
	init_global_connsys();

	// Wi-Fi is now ready - change OpMode immediately
	wifi_config_set_opmode(WIFI_MODE_AP_ONLY);
	//wifi_config_set_wireless_mode(WIFI_PORT_AP,WIFI_PHY_11A);
	//wifi_config_set_wireless_mode(WIFI_PORT_AP,WIFI_PHY_11AN_MIXED);
	wifi_config_set_wireless_mode(WIFI_PORT_AP,WIFI_PHY_11N_5G);
	wifi_config_set_country_region(WIFI_BAND_5_G,7);
	wifi_config_set_channel(WIFI_PORT_AP,channel);
	wifi_config_set_bandwidth(WIFI_PORT_AP,WIFI_IOT_COMMAND_CONFIG_BANDWIDTH_40MHZ);
	wifi_config_set_radio(1);
	setupWiFiConfig(ssid, passphrase, channel);
