...
int WiFiClass::begin_5G(const char* ssid)
{
	uint8_t status = WL_IDLE_STATUS;

	if (WiFiDrv::wifiSetNetwork_5G(ssid, strlen(ssid)) != WL_FAILURE)
	{
		status = waitForWiFiConnection();
	}
	else
...
int WiFiClass::begin_5G(const char* ssid, uint8_t key_idx, const char *key)
{
	uint8_t status = WL_IDLE_STATUS;

	// set encryption key
	if (WiFiDrv::wifiSetKey_5G(ssid, strlen(ssid), key_idx, key, strlen(key)) != WL_FAILURE)
	{
		status = waitForWiFiConnection();
	}
	else
...
int WiFiClass::begin_5G(const char* ssid, const char *passphrase)
{
	uint8_t status = WL_IDLE_STATUS;

	// set passphrase
	if (WiFiDrv::wifiSetPassphrase_5G(ssid, strlen(ssid), passphrase, strlen(passphrase)) != WL_FAILURE)
	{
		status = waitForWiFiConnection();
	}
	else
...
int8_t WiFiClass::scanNetworks_5G()
{
	uint8_t attempts = 10;
	uint8_t numOfNetworks = 0;

	if (WiFiDrv::startScanNetworks_5G() == WL_FAILURE)
	return WL_FAILURE;
	do
	{
		delay(500);
