...
int8_t set_net_5G(const char* ssid, uint8_t ssid_len)
{
	if (ssid_len == 0)
	return WL_FAILURE;
	...
	if (wifi_config_set_wireless_mode(port,WIFI_PHY_11AN_MIXED) < 0) {
		pr_debug("wifi_config_set_wireless_mode failed\r\n");
		goto err;
	}
	
	if (wifi_config_set_country_region(WIFI_BAND_5_G,7) < 0) {
		pr_debug("wifi_config_set_country_region failed\r\n");
		goto err;
	}
...
int8_t set_key_5G(const char *ssid, uint8_t ssid_len, uint8_t key_idx, const char *key, uint8_t len)
{
	if (ssid_len == 0 || len == 0)
	return WL_FAILURE;
...
int8_t set_passphrase_5G(const char* ssid, uint8_t ssid_len, const char *passphrase, uint8_t len)
{
	if (ssid_len == 0 || len == 0)
	return WL_FAILURE;
...
int8_t start_scan_net(void)
{
	...
	if (wifi_config_set_wireless_mode(WIFI_PORT_STA,WIFI_PHY_11BGN_MIXED) < 0) {
		pr_debug("wifi_config_set_wireless_mode failed\r\n");
	}
	
	if (wifi_config_set_country_region(WIFI_BAND_2_4_G,1) < 0) {
		pr_debug("wifi_config_set_country_region failed\r\n");
	}
...
int8_t start_scan_net_5G(void)
{
	int8_t status = 0, i;

	init_global_connsys();
	
	if (wifi_config_set_wireless_mode(WIFI_PORT_STA,WIFI_PHY_11AN_MIXED) < 0) {
		pr_debug("wifi_config_set_wireless_mode failed\r\n");
	}
