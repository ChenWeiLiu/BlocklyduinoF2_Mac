...
//----------------------------------------
void TFT_eSPI::ReInit_Pin()		//-111.07.24-CGH
{
	// The control pins are deliberately set to the inactive state (CS high) as setup()
	// might call and initialise other SPI peripherals which would could cause conflicts
	// if CS is floating or undefined.
...
void TFT_eSPI::init(uint8_t tc)
{
	if (_booted){	//-111.07.24-CGH
			...
			spi.begin(); // This will set HMISO to input
					...
					spi.begin(TFT_SCLK, TFT_MISO, TFT_MOSI, -1);
					// -> Serial.println("spi.begin(TFT_SCLK, TFT_MISO, TFT_MOSI, -1)");	//-111.06.27-Test
				#else
					spi.begin();
					// Serial.println("spi.begin()");	//-111.06.27-Test
				#endif
			#endif
		#endif
