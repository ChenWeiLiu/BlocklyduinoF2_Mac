...
void Adafruit_SoftServo::attach(uint8_t pin,uint16_t minPulse,uint16_t maxPulse) {
  servoPin = pin;
  angle = 90;
  isAttached = true;
  pinMode(servoPin, OUTPUT);
  minBandWidth=minPulse;
  maxBandWidth=maxPulse;
}
/**
...
void Adafruit_SoftServo::write(uint8_t a) {
  ...
  if (!isAttached)
    return;
  micros = map(a, 0, 180, minBandWidth, maxBandWidth);
  refresh();
  delay(15);
}

void Adafruit_SoftServo::write360(uint8_t a) {
  angle = a;
  if (!isAttached)
    return;
  if (a==90)
