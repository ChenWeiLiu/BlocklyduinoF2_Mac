...
template <class modemType> class TinyGsmModem {
  public:
     ...
     * Basic functions
     */
    bool                                    begin(const char *pin = NULL) { return thisModem().initImpl(pin); }
    bool                                    init(const char *pin = NULL) { return thisModem().initImpl(pin); }
    template <typename... Args> inline void sendAT(Args... cmd)
    {
        thisModem().streamWrite("AT", cmd..., thisModem().gsmNL);
        thisModem().stream.flush();
        TINY_GSM_YIELD(); /* DBG("### AT:", cmd...); */
    }
    void setBaud(uint32_t baud) { thisModem().setBaudImpl(baud); }
    // Test response to AT commands
    bool testAT(uint32_t timeout_ms = 10000L) { return thisModem().testATImpl(timeout_ms); }
    ...
    String getModemInfo() { return thisModem().getModemInfoImpl(); }
    ...
    bool radioOff() { return thisModem().radioOffImpl(); }
    bool sleepEnable(bool enable = true) { return thisModem().sleepEnableImpl(enable); }
    ...
    bool waitForNetwork(uint32_t timeout_ms = 60000L) { return thisModem().waitForNetworkImpl(timeout_ms); }
    // Gets signal quality report
    int16_t   getSignalQuality() { return thisModem().getSignalQualityImpl(); }
    String    getLocalIP() { return thisModem().getLocalIPImpl(); }
    IPAddress localIP() { return thisModem().TinyGsmIpFromString(thisModem().getLocalIP()); }
  ...
  protected:
    ...
    int8_t getRegistrationStatusXREG(const char *regCommand)
    {
        ...
        int8_t resp = thisModem().waitResponse(GF("+CREG:"), GF("+CGREG:"), GF("+CEREG:"));
  ...
  public:
    ...
    template <typename T> inline void streamWrite(T last) { thisModem().stream.print(last); }

    template <typename T, typename... Args> inline void streamWrite(T head, Args... tail)
    {
  ...
  protected:
    ...
    inline bool streamSkipUntil(const char c, const uint32_t timeout_ms = 1000L)
    {
        ...
        while (millis() - startMillis < timeout_ms) {
            while (millis() - startMillis < timeout_ms && !thisModem().stream.available()) {
                TINY_GSM_YIELD();
            }
