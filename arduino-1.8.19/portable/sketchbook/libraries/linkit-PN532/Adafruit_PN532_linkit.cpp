...
*/
/**************************************************************************/
Adafruit_PN532_linkit::Adafruit_PN532_linkit(uint8_t clk, uint8_t miso, uint8_t mosi, uint8_t ss):
  _clk(clk),
  _miso(miso),
  _mosi(mosi),
  _ss(ss),
...
*/
/**************************************************************************/
Adafruit_PN532_linkit::Adafruit_PN532_linkit(uint8_t irq, uint8_t reset):
  _clk(0),
  _miso(0),
  _mosi(0),
  _ss(0),
...
*/
/**************************************************************************/
Adafruit_PN532_linkit::Adafruit_PN532_linkit(uint8_t ss):
  _clk(0),
  _miso(0),
  _mosi(0),
  _ss(ss),
...
*/
/**************************************************************************/
void Adafruit_PN532_linkit::begin() {
  if (_usingSPI) {
    // SPI initialization
    if (_hardwareSPI) {
      SPI.begin();
...
*/
/**************************************************************************/
void Adafruit_PN532_linkit::PrintHex(const byte * data, const uint32_t numBytes)
{
  uint32_t szPos;
...
*/
/**************************************************************************/
void Adafruit_PN532_linkit::PrintHexChar(const byte * data, const uint32_t numBytes)
{
  uint32_t szPos;
  for (szPos=0; szPos < numBytes; szPos++)
  {
...
*/
/**************************************************************************/
uint32_t Adafruit_PN532_linkit::getFirmwareVersion(void) {
  uint32_t response;
...
bool Adafruit_PN532_linkit::sendCommandCheckAck(uint8_t *cmd, uint8_t cmdlen, uint16_t timeout) {
  uint16_t timer = 0;
...
*/
/**************************************************************************/
bool Adafruit_PN532_linkit::writeGPIO(uint8_t pinstate) {
  uint8_t errorbit;
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532_linkit::readGPIO(void) {
...
*/
/**************************************************************************/
bool Adafruit_PN532_linkit::SAMConfig(void) {
...
*/
/**************************************************************************/
bool Adafruit_PN532_linkit::setPassiveActivationRetries(uint8_t maxRetries) {
...
*/
/**************************************************************************/
bool Adafruit_PN532_linkit::readPassiveTargetID(uint8_t cardbaudrate, uint8_t * uid, uint8_t * uidLength, uint16_t timeout) {
...
*/
/**************************************************************************/
bool Adafruit_PN532_linkit::inDataExchange(uint8_t * send, uint8_t sendLength, uint8_t * response, uint8_t * responseLength) {
...
*/
/**************************************************************************/
bool Adafruit_PN532_linkit::inListPassiveTarget() {
  pn532_packetbuffer[0] = PN532_COMMAND_INLISTPASSIVETARGET;
...
*/
/**************************************************************************/
bool Adafruit_PN532_linkit::mifareclassic_IsFirstBlock (uint32_t uiBlock)
{
...
*/
/**************************************************************************/
bool Adafruit_PN532_linkit::mifareclassic_IsTrailerBlock (uint32_t uiBlock)
{
  // Test if we are in the small or big sectors
  if (uiBlock < 128)
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532_linkit::mifareclassic_AuthenticateBlock (uint8_t * uid, uint8_t uidLen, uint32_t blockNumber, uint8_t keyNumber, uint8_t * keyData)
{
  uint8_t len;
  uint8_t i;
  ...
  if (pn532_packetbuffer[7] != 0x00)
  {
      ...
      Adafruit_PN532_linkit::PrintHexChar(pn532_packetbuffer, 12);
    #endif
    return 0;
  }
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532_linkit::mifareclassic_ReadDataBlock (uint8_t blockNumber, uint8_t * data)
{
  #ifdef MIFAREDEBUG
  ...
  if (pn532_packetbuffer[7] != 0x00)
  {
      ...
      Adafruit_PN532_linkit::PrintHexChar(pn532_packetbuffer, 26);
    #endif
    return 0;
  }
  ...
  /* Display data for debug if requested */
    ...
    Adafruit_PN532_linkit::PrintHexChar(data, 16);
  #endif
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532_linkit::mifareclassic_WriteDataBlock (uint8_t blockNumber, uint8_t * data)
{
  #ifdef MIFAREDEBUG
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532_linkit::mifareclassic_FormatNDEF (void)
{
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532_linkit::mifareclassic_WriteNDEFURI (uint8_t sectorNumber, uint8_t uriIdentifier, const char * url)
{
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532_linkit::mifareultralight_ReadPage (uint8_t page, uint8_t * buffer)
{
  if (page >= 64)
  {
    #ifdef MIFAREDEBUG
  ...
  /* Display data for debug if requested */
    ...
    Adafruit_PN532_linkit::PrintHexChar(buffer, 4);
  #endif
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532_linkit::mifareultralight_WritePage (uint8_t page, uint8_t * data)
{
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532_linkit::ntag2xx_ReadPage (uint8_t page, uint8_t * buffer)
{
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532_linkit::ntag2xx_WritePage (uint8_t page, uint8_t * data)
{
  // TAG Type       PAGES   USER START    USER STOP
  // --------       -----   ----------    ---------
  // NTAG 203       42      4             39
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532_linkit::ntag2xx_WriteNDEFURI (uint8_t uriIdentifier, char * url, uint8_t dataLen)
{
...
*/
/**************************************************************************/
bool Adafruit_PN532_linkit::readack() {
  uint8_t ackbuff[6];
...
*/
/**************************************************************************/
bool Adafruit_PN532_linkit::isready() {
  if (_usingSPI) {
...
*/
/**************************************************************************/
bool Adafruit_PN532_linkit::waitready(uint16_t timeout) {
  uint16_t timer = 0;
  while(!isready()) {
    if (timeout != 0) {
      timer += 10;
...
*/
/**************************************************************************/
void Adafruit_PN532_linkit::readdata(uint8_t* buff, uint8_t n) {
  if (_usingSPI) {
    // SPI write.
    #ifdef SPI_HAS_TRANSACTION
      if (_hardwareSPI) SPI.beginTransaction(PN532_SPI_SETTING);
...
*/
/**************************************************************************/
void Adafruit_PN532_linkit::writecommand(uint8_t* cmd, uint8_t cmdlen) {
  if (_usingSPI) {
    // SPI command write.
    uint8_t checksum;
...
*/
/**************************************************************************/
void Adafruit_PN532_linkit::spi_write(uint8_t c) {
  if (_hardwareSPI) {
    // Hardware SPI write.
    SPI.transfer(c);
  }
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532_linkit::spi_read(void) {
  int8_t i, x;
  x = 0;
